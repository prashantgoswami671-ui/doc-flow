...
**Status: 🟡 Tier-1 Browser AI foundation and production Summarize PDF are implemented and verified. Tier-2 Ollama runtime exists but is NOT production-wired. T2-02→T2-09 research complete. Production Evidence Store layer (V6-B02→V6-B05) is IMPLEMENTED + VERIFIED but NOT YET WIRED into the generation flow. Stage-2 validation (V6-C) NOT STARTED. See task rows below for precise classification.**
...
- **Deterministic evidence validation now exists as a production layer (V6-B03, see next section) but is not yet wired into the production AI generation path.** The research validators remain fixture-only except as rebuilt in production by V6-B.

### Production Evidence Architecture / V6-B — ✅ IMPLEMENTED + VERIFIED (not yet wired into generation flow)

Implemented in `services/ai/evidence/` (provider-agnostic: no provider, network, DOM, or PDF imports). Local commits `91335e2` (V6-B02) → `a040e91` (V6-B03) → `979380e` (V6-B04) → `7bb2c59` (V6-B05); currently local only (origin/main still `62be2a5`). Evidence quartet: 4 test files / 144 tests passed; `services/ai` subset: 18 files / 382 passed; `npx tsc --noEmit --incremental false` exit 0.
...
**Production integration distinction (essential):** the V6-B layer above is IMPLEMENTED + VERIFIED as production code, but Stage-1 evidence acquisition and Stage-2 reasoning are NOT integrated into `services/ai/orchestration.ts` — the live generation flow is unchanged (PDF → AI-02 → instruction → `runtime.generateText()`). V6-C Stage-2 reasoning: NOT STARTED — no claim contract, no output validator, no grounded result projection, no comparison/attribution capability shipped, and no provider/UI changes were made as part of V6-B.
...
- V6-C status: NOT STARTED — no Stage-2 claim contract, output validator, or grounded result projection exists; see the V6-B section above for what the evidence layer does and does not cover.
...
**Right now:** Phase 4's SEC-01 through SEC-07 are done. Phase 5's Tier-1 Browser AI foundation (AI-01, AI-02, AI-16, AI-17, AI-21) and the production Summarize PDF integration (AI-18) are implemented and **READY TO CLOSE PHASE 1 / Tier-1 Browser AI** pending final verification, Brave browser QA, final review, commit, and push. Ollama Tier-2 provider-selection/UI/consent/production-integration, BYOK/Advanced (Tier 3), Translate/Key Points/Ask UIs, and Cloud AI remain not started (the Tier-2 Ollama runtime itself is implemented and verified — see T2-01).
